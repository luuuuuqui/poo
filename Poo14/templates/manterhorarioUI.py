...
class ManterHorarioUI:
    ...
    def listar():
        ...
        if len(horarios) == 0:
            ...
            profissionais = {
                p.get_id(): p.get_nome() for p in View.profissional_listar()
            }
            for obj in horarios:
                dic = {
                    "id": obj.get_id(),
                    ...
                    "profissional": profissionais.get(
                        obj.get_idprofissional(), obj.get_idprofissional()
                    ),
                }
                list_dic.append(dic)
    ...
    def inserir():
        ...
        if len(clientes) == 0:
            ...
            cliente = st.selectbox(
                "Selecione o cliente", clientes, format_func=lambda x: str(x)
            )
        ...
        if len(servicos) == 0:
            ...
            servico = st.selectbox(
                "Selecione o serviço", servicos, format_func=lambda x: str(x)
            )
    ...
    def atualizar():
        ...
        if len(horarios) == 0:
            ...
            if op is not None:
                ...
                profissionais = View.profissional_listar()
                profissional_index = next(
                    (
                        i
                        for i, p in enumerate(profissionais)
                        if p.get_id() == op.get_idprofissional()
                    ),
                    0,
                )
                profissional = st.selectbox(
    ...
    def excluir():
        ...
        if len(horarios) == 0:
            ...
            op = st.selectbox(
                "Exclusão de Horarios", horarios, format_func=lambda x: str(x)
            )
