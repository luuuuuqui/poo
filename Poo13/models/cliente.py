...
class Cliente:
    def __init__(self, id: int, nome: str, email: str, fone: str, senha: str, nascimento: datetime):
        ...
        if nascimento is None or isinstance(nascimento, datetime) == False or datetime.now() < nascimento:
            raise ValueError("Nascimento deve ser preenchido e ser uma data válida no passado.")
        self.set_id(id)
        self.set_nome(nome)
    ...
    def set_nascimento(self, nascimento) -> None: self.__nascimento = nascimento

    def to_json(self) -> dict:
        dic = {"id":self.__id, "nome":self.__nome, "email":self.__email, "fone":self.__fone, "senha":self.__senha, "nascimento":self.__nascimento}
        return dic
    ...
    def from_json(dic) -> object:
        ...
        return Cliente(dic["id"], dic["nome"], dic["email"], dic["fone"], dic["senha"], nascimento)
