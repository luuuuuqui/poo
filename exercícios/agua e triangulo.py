...
class Triangulo():
    def __init__(self):
        ...
        self.__h = 0
    def set_b(self, v):
        if v <= 0: raise ValueError("A base não pode ser igual ou menor que zero.")
        self.__b = v
    def set_h(self, v):
        if v <= 0: raise ValueError("A altura não pode ser igual ou menor que zero.")
        self.__h = v
    def get_b(self):
        return self.__b
    def get_h(self):
...
class UI: # UI = User Interface: print e input
    ...
    def triangulo():
        ...
        print(f"O triângulo de base {x.get_b()} e altura {x.get_h()} tem área {x.calc_area()}")
